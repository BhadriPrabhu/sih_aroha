    ...
    <DashboardLayout>
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">System & Forecast Recalibration</h1>
            <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">Backtesting historical Mean Absolute Error (MAE) to tune exponential smoothing parameters.</p>
          </div>
          <button
            onClick={handleRecalibrate}
            disabled={recalState === 'running'}
            className={`w-full sm:w-auto px-5 py-2.5 sm:py-2 rounded-md text-sm font-semibold shadow-sm transition-all flex items-center justify-center gap-2 sm:min-w-[210px] focus:outline-none focus:ring-2 focus:ring-offset-2 ${recalState === 'running'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6">
          ...
          <div className="col-span-1 lg:col-span-8 bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-slate-200 flex flex-col">
            <div className="mb-6 border-b border-slate-100 pb-4">
              <h3 className="text-base sm:text-lg font-bold text-slate-900">MAE Improvement by Category</h3>
              <p className="text-xs sm:text-sm font-medium text-slate-500">Comparing forecast error before and after parameter (α) tuning.</p>
            </div>
          ...
          <div className="col-span-1 lg:col-span-4 bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-slate-200 flex flex-col">
            <div className="border-b border-slate-100 pb-4 mb-4">
              <h3 className="text-base sm:text-lg font-bold text-slate-900">Alpha (α) Heuristics</h3>
              <p className="text-xs sm:text-sm font-medium text-slate-500">Current static weights applied to exponential smoothing.</p>
            </div>
...
function KpiCard({ title, value, suffix, subtitle, icon }) {
  return (
    <div className="bg-white p-4 sm:p-5 rounded-lg border border-slate-200 shadow-sm flex items-start gap-3 sm:gap-4">
      <div className="p-2 border border-slate-200 rounded-md bg-slate-50 text-slate-600 shrink-0">
        {icon}
      </div>
      <div>
        ...
        {subtitle && <p className="text-[11px] sm:text-[12px] text-slate-400 mt-1.5 font-medium leading-tight">{subtitle}</p>}
      </div>
    </div>
  );
}
...
function HeuristicRow({ label, desc, color }) {
  return (
    <div>
      ...
      <p className="text-xs sm:text-sm text-slate-600 pl-3.5 font-medium">{desc}</p>
    </div>
  );
}
...
function TableRow({ category, oldA, newA, metric, status }) {
  ...
  return (
    <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
      <td className="py-3 px-4 font-bold text-slate-900 whitespace-nowrap">{category}</td>
      <td className="py-3 px-4 font-mono text-slate-500 text-xs whitespace-nowrap">{oldA}</td>
      <td className="py-3 px-4 font-mono font-bold text-sky-600 text-xs whitespace-nowrap">{newA}</td>
      <td className="py-3 px-4 text-xs text-slate-600 font-medium whitespace-nowrap">{metric}</td>
      <td className="py-3 px-4 text-right">
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border whitespace-nowrap ${isApplied ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : 'text-slate-600 bg-slate-50 border-slate-200'}`}>
          {status}
        </span>
      </td>
    </tr>
